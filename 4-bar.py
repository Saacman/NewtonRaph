...
def velFunc(w_3, w_4):
    f1 = -a2 * sin(theta2) * w2 - a3 * sin(theta3_4[0][0]) * w_3 + a4 * sin(theta3_4[1][0]) * w_4
    f2 = a2 * cos(theta2) * w2 + a3 * cos(theta3_4[0][0]) * w_3 - a4 * cos(theta3_4[1][0]) * w_4
    return np.array([[f1], [f2]])

def updateVelValues(omegas):
    f = velFunc(omegas[0][0], omegas[1][0])
    print(f"Velocity Functions:\nf_w3: {f[0]}\nf_w4: {f[1]}")
    J = createJacobian(theta3_4[0][0], theta3_4[1][0])
    print(f"Jacobian:\n{J}")
...
def accFunc(alpha_3, alpha_4):
    f1 = -a2*sin(theta2)*alpha2 -a2*cos(theta2)*pow(w2, 2) -a3*sin(theta3_4[0][0])*alpha_3 -a3*cos(theta3_4[0][0])*pow(omegas3_4[0][0], 2) +a4*sin(theta3_4[1][0])*alpha_4 +a4*cos(theta3_4[1][0])*pow(omegas3_4[1][0], 2)
    f2 = a2*cos(theta2)*alpha2 -a2*sin(theta2)*pow(w2, 2) +a3*cos(theta3_4[0][0])*alpha_3 -a3*sin(theta3_4[0][0])*pow(omegas3_4[0][0], 2) -a4*cos(theta3_4[1][0])*alpha_4 +a4*sin(theta3_4[1][0])*pow(omegas3_4[1][0], 2)
    return np.array([[f1], [f2]])
...
if __name__ == '__main__':
    ...
    theta3_4 = np.array([[(14.0362 * pi) / 180], [(135* pi) / 180]])
    #Set initial velocity values (rad/s)
    omegas3_4 = np.array([[29.3333], [11.2941]])
    print("ACCELERATION")
